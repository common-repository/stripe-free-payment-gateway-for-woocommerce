=== Stripe free payment gateway for WooCommerce ===
Contributors: wpproducts
Donate link: https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=solvercircle@gmail.com&item_name=Donation+for+Stripe_free_payment_gateway_for_WooCommerce
Tags: Stripe payment gateway WooCommerce, Stripe free payment gateway, Stripe WooCommerce
Requires at least: 3.5.0.



== Description ==

This plugin extends WooCommerce allowing user to take payments directly on your store via Stripe API. Stripe payment is available in the United States, Canada, UK, Australia, France, Belgium, Netherlands, Germany, Irelandand more 


<h4>Others WooCommerce Payment module</h4>


<p>
<strong><a rel="nofollow" href="http://wordpress-expert.codeinterest.com/woo-commerce/woocommerce-paypal-payments-pro-direct-payment">WooCommerce PayPal Payments Pro (Direct Payment)</a></strong>,
 <strong><a rel="nofollow" href="http://wordpress-expert.codeinterest.com/woo-commerce/woocommerce-eway-payment-gateway">woocommerce eway payment gateway</a></strong>
 </p>

<p>Some of our popular extensions include 
<strong><a rel="nofollow" href="http://wordpress-expert.codeinterest.com/wordpress/icontact-wordpress">iContact Wordpress Plugin</a></strong>,
 <strong><a rel="nofollow" href="http://wordpress-expert.codeinterest.com/wordpress/get-response-wordpress">GetResponse Wordpress Plugin</a></strong>, 
 <strong><a rel="nofollow" href="http://wordpress-expert.codeinterest.com/woo-commerce/woocommerce-eway-payment-gateway">woocommerce eway payment gateway</a></strong>, 
 <strong><a rel="nofollow" href="http://wordpress-expert.codeinterest.com/woo-commerce/personalize-mobile-case-design-for-woocommerce">Personalize Mobile Case Design for WooCommerce</a></strong></p>



== Screenshots ==

1. Screenshot 1
2. Screenshot 2


== Installation ==

1) Upload 'woocommerce-stripe' folder to the '/wp-content/plugins/' directory
2) Activate 'woocommerce-stripe' form wp plugin option in admin area
3) Plugin will appear in the menu bar of WP Dashboard
4) set the plugin settings from WooCommerce checkout setting

== Changelog ==

= 1.0 =
First version of Stripe free payment gateway for WooCommerce grid. No errors known.